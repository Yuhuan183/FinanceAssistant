...
2. **ETF 穿透曝險（look-through）**：估算 QQQ／SMH／VOO／VT 等 ETF 內含的個股與產業權重，加總出組合對單一個股（特別是 NVDA／AMD／GOOGL）與單一產業（半導體、AI、科技）的「真實」曝險，點出帳面分散與實際集中度的落差、ETF 之間的重疊度。ETF 成分權重為估算值，須於報告中註明。
